...
linkml_meta = LinkMLMeta({'default_prefix': 'pp',
     ...
     'description': 'Domain-neutral viewpoint vocabulary for scientific-document '
                    'parsing. Declares document-level content kinds (sections, '
                    'figures, tables, citations, equations, claims, measurements, '
                    'evidence spans, extraction provenance) as pp:<term> CURIEs. '
                    'These are document semantics, not scientific-domain ontology: '
                    'no materials, biology, or chemistry concepts appear here. '
                    'ExtractionProfile layers reference these CURIEs in their '
                    'content-kind slots.',
     'id': 'https://w3id.org/grits/viewpoints/paper_parsing_v0',
     'imports': ['linkml:types', '../src/pygrits/core'],
     'license': 'BSD-3-Clause',
     'name': 'paper_parsing_v0',
     'prefixes': {'grits': {'prefix_prefix': 'grits',
                            'prefix_reference': 'https://w3id.org/grits/'},
                  'linkml': {'prefix_prefix': 'linkml',
                             'prefix_reference': 'https://w3id.org/linkml/'},
                  'pp': {'prefix_prefix': 'pp',
                         'prefix_reference': 'https://w3id.org/grits/viewpoints/paper_parsing_v0/'}},
     'source_file': 'viewpoints/paper_parsing_v0.yaml',
     'title': 'Generic Paper Parsing Viewpoint v0'} )
...
class PaperParsingContentKind(str, Enum):
    """
    Document-level content kinds for scientific-paper parsing. Emitted as pp:<term> CURIEs and referenced by ExtractionProfile content-kind slots (preserve_content_kinds, require_grounding_for_content_kinds, high_fidelity_content_kinds) and by EvidenceRecord.evidence_type.
    """
    section = "section"
    """
    A document section or subsection.
    """
    figure = "figure"
    """
    ...
    citation = "citation"
    """
    A bibliographic citation or reference.
    """
    equation = "equation"
    """
    ...
    claim = "claim"
    """
    An asserted statement reported by the document.
    """
    measurement = "measurement"
    """
    A reported measured or computed value.
    """
    evidence_span = "evidence_span"
    """
    A verbatim span serving as grounding evidence.
    """
    extraction_provenance = "extraction_provenance"
    """
    A record of how content was extracted (tool, model, pass).
    """
...
class PaperParsingScope(NotesOnlyScope):
    """
    Scope for generic paper parsing; no scientific-domain dimensions.
    """
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta({'from_schema': 'https://w3id.org/grits/viewpoints/paper_parsing_v0'})

    scope_type: Literal["PaperParsingScope"] = Field(default="PaperParsingScope", description="""Class name of the concrete Scope subclass (e.g. NotesOnlyScope).""", json_schema_extra = { "linkml_meta": {'designates_type': True, 'domain_of': ['Scope']} })
    notes: Optional[str] = Field(default=None, description="""Free-text scope clarification.""", json_schema_extra = { "linkml_meta": {'domain_of': ['Scope', 'ReasoningPolicy']} })
...
PaperParsingScope.model_rebuild()
