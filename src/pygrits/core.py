...
linkml_meta = LinkMLMeta({'default_prefix': 'isom',
     ...
     'description': 'Schema for the Interactive Scientific Object Model (ISOM).\n'
                    ...
                    'primitives (ContentReference, Scope, Locator hierarchy, '
                    'structured Confidence). All entities share the discipline '
                    'contract (identity, type, viewpoint, provenance, '
                    'should_not_claim) and differ by role. The schema implements '
                    'the ISOM spec v0.\n'
                    'This is a v0.1 draft schema covering the vertical slice '
                    'needed for the first implementation milestone (NaCl-style '
                    'epistemically-disciplined refusal). It does not yet model '
                    'speech acts, reactions, threads, communities, wiki statement '
                    'objects, or the harmonization process. Those are deferred to '
                    'v0.2.',
...
class EvidenceTypeBase(str, Enum):
    """
    Built-in evidence categories. Viewpoints may extend evidence_type with additional CURIEs, but these base values are structural and always available.
    """
    text_span = "text_span"
    figure = "figure"
    table = "table"
    ...
    processing_statistic = "processing_statistic"
    parameter = "parameter"
...
class ThermodynamicScope(ConfiguredBaseModel):
    """
    ...
    notes: Optional[str] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['ThermodynamicScope', 'Scope']} })
...
class BiologicalScope(ConfiguredBaseModel):
    """
...
class CompositionalScope(ConfiguredBaseModel):
    """
...
class StatisticalScope(ConfiguredBaseModel):
    """
...
class MethodologicalScope(ConfiguredBaseModel):
    """
...
class SpatialScope(ConfiguredBaseModel):
    """
...
class Scope(ConfiguredBaseModel):
    """
    Composite scope. Each dimension is optional. An unset dimension is operationally `unspecified` — not a wildcard, but an honest acknowledgement that the viewpoint did not bind it.
    """
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta({'from_schema': 'https://w3id.org/isom/core'})

    thermodynamic: Optional[ThermodynamicScope] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['Scope']} })
    temporal: Optional[TemporalScope] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['Scope']} })
    biological: Optional[BiologicalScope] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['Scope']} })
    compositional: Optional[CompositionalScope] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['Scope']} })
    statistical: Optional[StatisticalScope] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['Scope']} })
    methodological: Optional[MethodologicalScope] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['Scope']} })
    spatial: Optional[SpatialScope] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['Scope']} })
    notes: Optional[str] = Field(default=None, description="""Free-text scope clarification.""", json_schema_extra = { "linkml_meta": {'domain_of': ['ThermodynamicScope', 'Scope']} })


class Locator(ConfiguredBaseModel):
    """
    Polymorphic locator into a source artifact. Discriminator is locator_type.
    """
...
class Entity(ConfiguredBaseModel):
    ...
    scope: Optional[Scope] = Field(default=None, description="""Optional but recommended. Sets the conditions under which this entity's statements apply. Unset dimensions are operationally `unspecified`, not wildcards.""", json_schema_extra = { "linkml_meta": {'domain_of': ['Entity']} })
...
class EvidenceRecord(Entity):
    ...
    evidence_type: Optional[str] = Field(default=None, description="""Either a value from EvidenceTypeBase (e.g. isom:text_span) or a CURIE into a viewpoint-extended evidence vocabulary.""", json_schema_extra = { "linkml_meta": {'domain_of': ['EvidenceRecord']} })
