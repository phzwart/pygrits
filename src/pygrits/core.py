...
linkml_meta = LinkMLMeta({'default_prefix': 'grits',
     ...
     'description': 'Minimal evidence-grounded node model on PROV-O. Three roles: '
                    'Entity, Activity, EvidenceRecord. Domain content is added by '
                    'importing schemas that subclass Entity/Activity with typed '
                    'slots. No domain vocabulary here.',
...
class Grit(ConfiguredBaseModel):
    ...
    scope: Optional[Union[Scope,NotesOnlyScope]] = Field(default=None, json_schema_extra = { "linkml_meta": {'domain_of': ['Grit']} })
